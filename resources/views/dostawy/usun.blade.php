                ...
                <div class="card">
                    <div class="card-header">
                        <table>
                            <tbody>
                            <tr>
                                <td width="30%">{{ __('Usuń stan zaopatrzenia dla')}}</td>
                                <td width=40%">
                                    <form method="POST"
                                          action="{{ route('dostawy.wybierzUsun') }}">
                                        @csrf
                    ...
                    <form method="POST" action="{{ route('dostawy.usunZBazy') }}">
                        @csrf
                        <div class="card-body">
                            @if (session('status'))
                                ...
                                @if(isset($dostawy))
                                    @foreach($dostawy as $produkt)
                                        <tr>
                                            ...
                                            <td>{{ $produkt->ilość }}</td>
                                            <td></td>
                                            <td><input type="number" value={{ old("produkt".$produkt->produkt_id) ?? "0" }} min="0" max={{$produkt->ilość}} id={{ "produkt".$produkt->produkt_id }} name={{ "produkt".$produkt->produkt_id }}
                                            ...
                                            <div class="row mb-0">
                                                <div class="col-md-8 offset-md-4">
                                                    <button type="submit" class="btn btn-primary"
                                                            onclick="return confirm('Czy jesteś pewny, że chcesz usunąć te produkty?')">
                                                        {{ __('Usuń') }}
                                                    </button>
                                                </div>
                                            </div>
